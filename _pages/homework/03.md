...
programming:
  ...
  description: |
    ...
    Using the ```if()``` command and p5.js variables for [detecting mouse presses](https://p5js.org/reference/#EventsMouse), create an interactive piece that draws something different every time there's a mouse click. Which shapes to draw is up to you: they can be simple ```rect()``` of different proportions, ```line()``` of varying widths or even ```arcs()``` and [```bezier()```](https://p5js.org/reference/#/p5/bezier) curves with ```random()``` parameters. Their location should always be related to where the click was detected on the canvas, and their colors and other parameters should always be different. You can use just one shape for the whole exercise (as long as the parameters are always different) or you can vary the shape with each click.
